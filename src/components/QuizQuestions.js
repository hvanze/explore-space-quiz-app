...
export default function Question({ question, changeQuestion }) {
    ...
    return (
        <div className='d-flex bg-dark text-white text-start'>
            <h2 id='question' dangerouslySetInnerHTML={{ __html: question.question }}></h2>
            {question.answerChoices.map((choice, index) => (
                <div key={index} className={`choice-container ${selectedAnswer === index && classToApply}`}
                    onClick={() => checkAnswer(index)}>
                    <p className="choice-prefix">{index + 1}</p>
                    <p className="choice-text" dangerouslySetInnerHTML={{ __html: choice }}></p>
                </div>
            ))}
            <Link to="/" className="exit-to-main-menu">X</Link>
        </div>
    );
}
